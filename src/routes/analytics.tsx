...
export const Route = createFileRoute("/analytics")({
  head: () => ({
    meta: [
      { title: "Impact Analytics — FoodSaver" },
      { name: "description", content: "Meals served, food waste reduced, CO₂ saved, and compliance logs." },
    ],
  }),
  component: Analytics,
});
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Impact analytics</p>
          <h1 className="mt-1 font-display text-3xl font-bold tracking-tight sm:text-4xl">Every meal counts</h1>
        </motion.div>

        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard icon={Activity} label="Meals served" value="48,210" hint="last 90 days" tint="success" />
          <StatCard icon={Leaf} label="CO₂ saved" value="62.8 t" hint="vs landfill baseline" tint="primary" delay={0.05} />
          <StatCard icon={Users} label="Active volunteers" value="1,420" hint="+12% this month" tint="info" delay={0.1} />
          <StatCard icon={Globe} label="Cities covered" value="12" hint="across 4 states" tint="warning" delay={0.15} />
        </div>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
            ...
            <div className="mt-4 h-72">
              <ResponsiveContainer>
                <LineChart data={trend} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
                  ...
                  />
                  <Legend />
                  <Line type="monotone" dataKey="meals" stroke="var(--chart-1)" strokeWidth={3} dot={{ r: 4 }} />
                  <Line type="monotone" dataKey="kg" stroke="var(--chart-2)" strokeWidth={3} dot={{ r: 4 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="mt-4 h-72">
              <ResponsiveContainer>
                <PieChart>
                  <Pie data={categories} dataKey="value" innerRadius={50} outerRadius={90} paddingAngle={3}>
                    {categories.map((_, i) => (
              ...
              {donations.slice(0, 5).map((d) => (
                <li key={d.id} className="rounded-2xl border border-border/60 bg-card p-3 text-xs">
                  ...
                  <p className="mt-1 text-muted-foreground">
                    Prepared {format(new Date(d.compliance.preparedAt), "p, MMM d")} · {d.compliance.storage}
                  </p>
