...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Connect donors, volunteers, and NGOs to rescue surplus food and feed communities. Real-time matching, compliance logs, and impact analytics." },
      { property: "og:title", content: "FoodSaver — Turn surplus food into shared meals" },
      { property: "og:description", content: "Real-time food rescue platform connecting donors, volunteers, and NGOs." },
    ],
  }),
  component: Landing,
});
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24">
          <div className="flex flex-col justify-center">
            ...
            >
              Turn surplus food into <span className="bg-gradient-to-r from-primary to-primary-glow bg-clip-text text-transparent">shared meals</span>.
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              FoodSaver connects restaurants, volunteers and NGOs in real time — so good food reaches people, not landfills.
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Link to="/dashboard" className="group inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-glow transition hover:opacity-90">
                Open dashboard <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Link>
              <Link to="/analytics" className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-6 py-3 text-sm font-semibold transition hover:bg-muted">
                See impact
              </Link>
            </motion.div>
          ...
          >
            ...
            >
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Live alert</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
        <h2 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">How FoodSaver works</h2>
        <p className="mt-2 max-w-xl text-muted-foreground">Three roles. One mission. Zero friction.</p>
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {[
            { icon: HandHeart, title: "Donors post surplus", body: "Restaurants & kitchens log surplus food in seconds with full compliance details." },
            { icon: Truck, title: "Volunteers dispatch", body: "Nearby volunteers get instant alerts and accept the closest pickup." },
            { icon: Users, title: "NGOs deliver impact", body: "Track meals served, CO₂ saved, and tax-deductible value in real time." },
          ].map((s, i) => (
            <motion.div
              key={s.title}
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-8 text-sm text-muted-foreground sm:px-6">
          <div className="flex items-center gap-2"><Leaf className="h-4 w-4" /> FoodSaver © {new Date().getFullYear()}</div>
