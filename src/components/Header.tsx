...
import { useStore, type Role } from "@/lib/mockStore";
...
export function Header() {
  const { role, setRole, notifications, markAllRead, user } = useStore();
    ...
    <header className="sticky top-0 z-40 border-b border-border/50 bg-background/70 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2">
            ...
            {roles.map((r) => (
              <button
                ...
                className={cn(
                  ...
                  role === r.value ? "bg-card text-foreground shadow-soft" : "text-muted-foreground hover:text-foreground"
                )}
              >
                {r.label}
              </button>
          ...
          <div className="hidden h-10 items-center gap-2 rounded-full bg-muted px-3 sm:flex">
